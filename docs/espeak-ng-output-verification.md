...
| 同綴異音語正解率 | 約71%（本レポート3節） | 未実装（E4で対応予定） |
| ライセンス | GPL v3 | Apache-2.0 |

#### 今後の対策（E3/E4で対応予定）

本レポートで確認されたespeak-ngの弱みに対し、以下のマイルストーンで段階的に対処する:

4. **テキスト正規化（E3）**: espeak-ngが弱い `$99.99` の通貨読み（$記号が無視される）や `2026-03-05` の日付フォーマット認識を、`EnglishNormalizer` モジュールで対応予定。数字→英語読み変換（基数・序数・小数・通貨・時刻）、略語展開、記号→名前変換を実装する
5. **同綴異音語対応（E4）**: espeak-ngでも正解率約71%にとどまる同綴異音語判別（read/lead/bow等）に対し、品詞ルールベースの `HomographResolver` で70%以上の正解率を目標とする。接尾辞ルール（-ing→動詞, -tion→名詞等）と位置ルール（冠詞後は名詞等）による軽量な品詞推定で判別する
